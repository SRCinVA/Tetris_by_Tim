        ...
        if (0,0,0) not in row:
            ...
            for j in range(len(row)):
                try:
                    ...
                    continue
    if inc > 0:
        for key in sorted(list(locked), key = lambda x: x [1]) [::-1]: #go backwards to not overwrite lower-positioned rows.
            x,y = key                                                  #remember: you cna overwrite keys in a dictionary.
            if y < ind:
                newKey = (x,y + inc)
                locked[newKey] = locked.pop(key)

    return inc   #number of rows cleared, as a stand-in for score.
...
def update_score(nscore):
    ...
    with open('score.txt', 'w') as f: #to add to the score
        if int(score) > nscore:
            f.write(str(score))
        else:
            f.write(str(nscore))

        score = lines[0].strip()  # to strip off the invisible character.
...
def main(win):
    ...
    while run:
        ...
        if level_time/1000 > 5:
            level_time = 0
            if fall_speed > 0.12: #so the game is still playable. This is "terminal velocity".
                fall_speed -= 0.005
